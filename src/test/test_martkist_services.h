...
#ifndef MARTKIST_TEST_TEST_MARTKIST_SERVICES_H
#define MARTKIST_TEST_TEST_MARTKIST_SERVICES_H
...
/** Testing martkist services setup that configures a complete environment with 3 nodes.
 */
...
const string OfferNew(const string& node, const string& aliasname, const string& category, const string& title, const string& qty, const string& price, const string& description, const string& currency, const string& certguid="''", const string& paymentoptions="MARTK",  const string& offerType = "BUYNOW", const string& auction_expires = "0", const string& auction_reserve = "0", const string& auction_require_witness = "false", const string &auction_deposit = "0", const string& witness="''");
...
struct MartkistTestingSetup {
    MartkistTestingSetup();
    ~MartkistTestingSetup();
};
struct BasicMartkistTestingSetup {
    BasicMartkistTestingSetup();
    ~BasicMartkistTestingSetup();
};
struct MartkistMainNetSetup {
	MartkistMainNetSetup();
	~MartkistMainNetSetup();
};
#endif
