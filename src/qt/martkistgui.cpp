...
const std::string MartkistGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MartkistGUI::DEFAULT_WALLET = "~Default";

MartkistGUI::MartkistGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void MartkistGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and martkist: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Martkist Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Martkist addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Martkist addresses"));
    ...
    openAction->setStatusTip(tr("Open a martkist: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Martkist command-line options").arg(tr(PACKAGE_NAME)));
...
void MartkistGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void MartkistGUI::createToolBars()
{
#ifdef ENABLE_WALLET
    if(walletFrame)
    {
...
void MartkistGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MartkistGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MartkistGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MartkistGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MartkistGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void MartkistGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void MartkistGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MartkistGUI::optionsClicked()
{
...
void MartkistGUI::showDebugWindow()
{
...
void MartkistGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void MartkistGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MartkistGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void MartkistGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void MartkistGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void MartkistGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void MartkistGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void MartkistGUI::showBackups()
{
    GUIUtil::showBackups();
}

void MartkistGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void MartkistGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void MartkistGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
...
void MartkistGUI::gotoMasternodePage()
{
    QSettings settings;
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool() && masternodeAction) {
...
void MartkistGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MartkistGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MartkistGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MartkistGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void MartkistGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Martkist network", "", count));
    } else {
...
void MartkistGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MartkistGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MartkistGUI::updateHeadersSyncProgressLabel()
{
...
void MartkistGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in martkist
        {
...
void MartkistGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void MartkistGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Martkist Core"); // default title
...
void MartkistGUI::changeEvent(QEvent *e)
{
...
void MartkistGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MartkistGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MartkistGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(MartkistUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void MartkistGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MartkistGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MartkistGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MartkistGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MartkistGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void MartkistGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void MartkistGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MartkistGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MartkistGUI::detectShutdown()
{
...
void MartkistGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MartkistGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MartkistGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MartkistGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MartkistGUI::subscribeToCoreSignals()
{
...
void MartkistGUI::unsubscribeFromCoreSignals()
{
...
void MartkistGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MartkistGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<MartkistUnits::Unit> units = MartkistUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const MartkistUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(MartkistUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(MartkistUnits::Unit u, MartkistUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MartkistUnits::name(u)), this);
...
{
    setText(MartkistUnits::name(newUnits));
}
