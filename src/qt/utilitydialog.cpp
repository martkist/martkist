    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (helpMode == about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  martkist-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_MARTKIST_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MartkistGUI::DEFAULT_UIPLATFORM));
        }
...
All the Martkist in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.1 MARTK, 1 MARTK, 10 MARTK and 100 MARTK -- sort of like the paper money you use every day.</li> \
...
For more info see <a href=\"https://martkistpay.atlassian.net/wiki/display/DOC/PrivateSend\">https://martkistpay.atlassian.net/wiki/display/DOC/PrivateSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
...
QWidget *ShutdownWindow::showShutdownWindow(MartkistGUI *window)
{
    if (!window)
        return nullptr;
