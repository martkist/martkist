 ...
 *
 *   // network
 *   CMartkistNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.martkist.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CMartkistNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CMartkistNetwork(UniValue objIn)
    {
...
// // can be under: MartkistNetwork
// //   -- signature requirements : Key1(User)
// class CMartkistNetworkVariable : public CGovernanceObject
// {
// private:
